...
impl Window {
	/// # General Bindings.
	///
	/// Set up general event bindings. These are actions that can be entirely
	/// handled by GTK methods.
	fn bindings(&self) {
		// Close down with the window.
		self.wnd_main.connect_delete_event(|_, _| {
			gtk::main_quit();
			Inhibit(false)
		});

		// Close from quit menu.
		self.mnu_quit.connect_activate(glib::clone!(
			@weak self.wnd_main as wnd_main => move |_| { wnd_main.close(); }
		));

		// Keep the status log scrolled to the end.
		self.lbl_status.connect_size_allocate(glib::clone!(
			@weak self.wnd_status as wnd_status => move |_, _| {
				if let Some(adj) = wnd_status.get_vadjustment() {
					adj.set_value(adj.get_upper());
				}
			}
		));

		// Sync display of ab/format/quality fields with `lbl_quality`. This
		// way we can toggle the latter, and all the others will follow.
		macro_rules! preview_cb {
			($hook:ident, $action:ident, $opacity:literal) => (
				self.lbl_quality.$hook(glib::clone!(
					@weak self.box_ab as box_ab,
					@weak self.lbl_format as lbl_format,
					@weak self.lbl_format_val as lbl_format_val,
					@weak self.lbl_quality_val as lbl_quality_val
					=> move |_| {
						box_ab.set_opacity($opacity);
						lbl_format.set_opacity($opacity);
						lbl_format_val.$action();
						lbl_quality_val.$action();
					}
				));
			);
		}
		...
		// Keep people from disabling both lossy and lossless modes.
		macro_rules! mode_cb {
			($main:ident, $other:ident) => (
				self.$main.connect_toggled(glib::clone!(
					@weak self.$other as $other => move |btn| {
						if ! btn.get_active() && ! $other.get_active() {
							btn.set_active(true);
						}
					}
				));
		...
		mode_cb!(chk_lossless, chk_lossy);
		mode_cb!(chk_lossy, chk_lossless);

		// Keep people from disabling every encoder.
		macro_rules! encode_cb {
			($main:ident, $other1:ident, $other2:ident) => (
				self.$main.connect_toggled(glib::clone!(
					@weak self.$other1 as $other1,
					@weak self.$other2 as $other2 => move |btn| {
						if ! btn.get_active() && ! $other1.get_active() && ! $other2.get_active() {
							btn.set_active(true);
						}
					}
				));
		...
		encode_cb!(chk_avif, chk_jxl, chk_webp);
		encode_cb!(chk_jxl, chk_avif, chk_webp);
		encode_cb!(chk_webp, chk_jxl, chk_avif);

		// The about menu.
		self.mnu_about.connect_activate(glib::clone!(
			@weak self.wnd_main as wnd_main => move |_| {
				// Build the dialogue.
				let about = gtk::AboutDialogBuilder::new()
					.attached_to(&wnd_main)
					.authors(vec![
				...
				// Show it and handle its closure.
				if ResponseType::None != about.run() { about.emit_close(); }
			}
		));
	}
}
